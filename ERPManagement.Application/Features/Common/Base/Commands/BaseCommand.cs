...
{
	public class BaseCommand<T, TViewModel>
		: BaseResponseHandler
		 where T : class
...

	{
		...
		public BaseCommand(IBaseRepository<T> branchRepository, IMapper mapper, IUnitOfwork unitOfwork)
		{
		...
		public async Task<T> CreateHandle(TViewModel request, CancellationToken cancellationToken)
		{
			try
			{
		...
		public async Task<bool> DeleteHandle(int id, CancellationToken cancellationToken)
		{
